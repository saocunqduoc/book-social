...
import { getToken, getLoginUserId } from '../services/localStorageService';
...
const MyProfile = () => {
  ...
  const fetchPosts = async () => {
    ...
    try {
        const response = await axios.get(`http://localhost:8668/api/v1/post/${loginUserId}/posts`, {
            headers: {
        ...
        postsData.forEach(post => {
            updatedLikedPosts[post.id] = post.likes.some(like => like.userId === loginUserId);
    ...
    useEffect(() => {
      ...
      socketRef.current.onmessage = (event) => {
          ...
          switch (message.type) {
              case 'like':
                  const likedPostId = message.data.postId;
                  setLikes((prevLikes) => ({
                      ...prevLikes,
                      [likedPostId]: (prevLikes[likedPostId] || 0) + 1,
                  }));
                  setLikedPosts((prev) => ({ ...prev, [likedPostId]: true }));
                  break;
  
              case 'unlike':
                  const unlikedPostId = message.data.postId;
                  setLikes((prevLikes) => ({
                      ...prevLikes,
                      [unlikedPostId]: Math.max((prevLikes[unlikedPostId] || 1) - 1, 0),
                  }));
                  setLikedPosts((prev) => ({ ...prev, [unlikedPostId]: false }));
                  break;
    ...
    const handleLikePost = async (postId) => {
      ...
      try {
          ...
          // Gửi thông điệp đến WebSocket với userId
